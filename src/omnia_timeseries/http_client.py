...
def _request(
    ...
    if headers is not None and "Accept" not in headers or headers["Accept"] == "application/json":
...
class HttpClient:
    def __init__(
        ...
        if resource_id is None or not isinstance(resource_id, str):
            raise ValueError("Input resource id must be a valid Azure application (client) id")
